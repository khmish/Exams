...
function createTableElement() {
	...
	result.className = "table table-dark";
	return result;
}
...
function createTableHeader(table, parseResult) {
	...
	for (var i = 0; i < parseResult.variables.length; i++) {
		...
		cell.innerHTML = "<h4>"+parseResult.variables[i]+"</h4>";
	...
	lastCell.innerHTML = "<h4>"+parseResult.ast.toString(parseResult.variables)+"</h4>";
...
function outputRow(table) {
	return function(assignment, result) {
		...
		for (var i = 0; i < assignment.length; i++) {
			...
			cell.innerHTML = (assignment[i]? "<h4>T</h4>" : "<h4>F</h4>");
