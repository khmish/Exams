    ...
    if (this.txt1 && this.txt2) {
      ...
      if (this.A && this.B) {
        if (this.radioValue == 'add') {
          ...
          if (this.A.rows == this.B.rows && this.A.columns == this.B.columns) {
            let c = Matrix.add(this.A, this.B);
            this.solutions = this.printResult(c)
          } else {
            this.solutions = "<h4> no sulotion because A size not equals B </h4>"
          }
          ...
          if (this.A.columns == this.B.rows) {

            let c = this.A.mmul(this.B);
            this.solutions = this.printResult(c)
          } else {
            this.solutions = "<h4> no sulotion because A rows not equals B columns </h4>"
          }
        ...
        else if (this.radioValue == 'inv') {
          this.solutionsB = ""

          let c = inverse(this.A,true);
          this.solutions = this.printResult(c)
        }  
        else {
          this.solutionsB = ""
...
let array1=[];
    ...
    for (let index = 0; index < ar.length; index++) {
      array1[index] = ar[index].split(',');

    }
    var A = new Matrix(array1);
